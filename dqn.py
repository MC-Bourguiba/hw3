...
def learn(env,
    ...
    q_func_net=q_func(obs_t_float,num_actions,"q_func",False)
    target_q_func_net=q_func(obs_tp1_float,num_actions,"target_q_func",False)
    target_y=rew_t_ph+gamma*tf.reduce_max(target_q_func_net,axis=1)
    ...
    outF = open('output'+str(task_id)+'nn'+str(nn_size)+str(target_update_freq), 'a')
    ...
    for t in itertools.count():
        ...
        latest_input=replay_buffer.encode_recent_observation()
        shape=[1]+list(input_shape)
        latest_input=array(latest_input).reshape(shape)
        if model_initialized:

            epsilon = 0.01
            q_values=session.run(q_func_net,feed_dict={obs_t_ph:latest_input})
        ...
        if (t > learning_starts and
            ...
            session.run(train_fn,feed_dict={obs_t_ph:obs_t_batch,act_t_ph:act_batch,
                                            rew_t_ph:rew_batch,obs_tp1_ph:obs_tp1_batch,
